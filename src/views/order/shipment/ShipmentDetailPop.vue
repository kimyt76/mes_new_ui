...
const deleteShipmentItemIds = ref([])
...
const saveInfo = async () =>{
    ...
    try{
        ...
        const request = {
            ...
            deleteShipmentItemIds: deleteShipmentItemIds.value,
        ...
        const res = await ApiOrder.updateShipmentInfo(formData)
        vSuccess("정상적으로 수정되었습니다.")
        closeDialog()
    }catch(err){
        handleApiError(err)
        return;
...
const removeRow = (idx) =>{
    ...
    if (row.weighInvId) {
        deleteShipmentItemIds.value.push(row.shipmentItemId)
    }
...
onMounted( async () =>{
    ...
    shipmentItemList.value = res.shipmentItemList
