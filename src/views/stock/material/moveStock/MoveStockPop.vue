...
const saveInfo = async () =>{
    ...
    if( moveStockList.value.length <= 0 ) return vWarning('이동 요청할 품목을 추가해주세요.')
...
const addRow = (row) =>{
  ...
  if (moveStockList.value.length > 0) {
    moveStockList.value.push(...rowItem);
  } else {
    moveStockList.value = [...rowItem];
  }
}
