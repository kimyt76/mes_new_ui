...
<form @submit.prevent="srhList" class="space-y-4">
    <Toolbar class="flex flex-wrap mt-2 mb-2 gap-1 w-full"  >
        <template #start>
            ...
            <FloatLabel variant="on">
                <DatePicker v-model="form.stdDate" inputId="on_label" showIcon iconDisplay="input" />
                <label for="on_label">기준일자</label>
            </FloatLabel>
...
<div>
    <DataTable
        ...
        <Column field="testNo"      header="시험번호"  frozen :style="{ width: '120px', textAlign:'center'}" :pt="{ columnHeaderContent: 'justify-center' }"/>
        <Column field="useByDate"   header="사용기한"  frozen  :style="{ width: '300px'}" bodyClass="break-words" :pt="{ columnHeaderContent: 'justify-center' }" />
        <Column field="inPrice"     header="잔여일"  frozen :pt="{ columnHeaderContent: 'justify-center' }" :style="{ width: '100px', textAlign:'right'}">
            <template #body="slotProps">{{ Number(slotProps.data.inPrice).toLocaleString() }}</template>
        </Column>
        <Column field="orderQty"        header="합계"  :pt="{ columnHeaderContent: 'justify-center' }" :style="{ width: '120px', textAlign:'right'}" >
            <template #body="slotProps">{{ Number(slotProps.data.orderQty).toLocaleString() }}</template>
        </Column>

        <!-- 가변적으로 변경해야함 -->
        <Column field="orderAmt"        header="[시화]원재료창고"  :pt="{ columnHeaderContent: 'justify-center' }" :style="{ width: '120px', textAlign:'right'}" >
            <template #body="slotProps">{{ Number(slotProps.data.orderQty).toLocaleString() }}</template>
        </Column>
        <Column field="inQty"        header="[시화]2층칭량실"  :pt="{ columnHeaderContent: 'justify-center' }" :style="{ width: '100px', textAlign:'right'}" >
            <template #body="slotProps">{{ Number(slotProps.data.orderQty).toLocaleString() }}</template>
        </Column>
        <Column field="inAmt"        header="[시화]3층칭량실"  :pt="{ columnHeaderContent: 'justify-center' }" :style="{ width: '100px', textAlign:'right'}" >
            <template #body="slotProps">{{ Number(slotProps.data.orderQty).toLocaleString() }}</template>
        </Column>
...
const downloadExcel = () =>{
  ...
  exportToExcel(rawMatList.value, "사용기한 리스트", cols);
}
