...
<div>
    <DataTable
        ...
        class="my-table"
        >
        <Column field="orderType"       header="관리구분"  frozen :style="{ width: '90px', textAlign:'center'}" />
        <Column field="itemTypeName"    header="품목구분"  frozen :style="{ width: '90px', textAlign:'center'}" />
        <Column field="itemCd"          header="품목코드"  frozen :style="{ width: '130px', textAlign:'center'}" />
        <Column field="itemName"        header="품목명"    frozen :style="{ width: '400px'}" bodyClass="break-words"   />
        <Column field="customerName"    header="납품업체"  frozen  :style="{ width: '280px'}" bodyClass="break-words"  />
        ...
        <Column field="xsQty"        header="매출조정수량"   :style="{ width: '100px', textAlign:'right'}" >
...
const srhList = async () =>{
    ...
    // api
    subMatList.value = await ApiStock.getTranLedger(params)
     console.log('subMatList.value', subMatList.value)
}
