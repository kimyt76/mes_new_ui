        ...
        <div class="search-section">
          <form @submit.prevent="searchList" class="search-form">
            <div class="search-field flex gap-3">
                ...
                <FloatLabel variant="on">
                    <InputText id="on_label1" v-model="form.testNo" style="width: 180px" />
                    <label for="on_label1">시험번호</label>
                </FloatLabel>
                <Button label="검색" icon="pi pi-search" severity="secondary" type="submit" class="search-button" />
            </div>
          </form>
        </div>
        <!-- 🔹 리스트 영역 (높이 제한) -->
        <div class="list-section mt-3">
          <div class="list-wrapper">
            ...
            >
              ...
              <Column field="testNo"            header="시험번호":style="{ width: '120px'}"style="text-align: center;"/>
              <Column field="qty"               header="수량":style="{ width: '120px'}"style="text-align: center;"/>
              <Column field="storageName"       header="창고":style="{ width: '150px'}"style="text-align: center;"/>
            </DataTable>
          </div>
        </div>
...
const searchList = async () =>{
    ...
    stockItemList.value = await ApiStock.getInvTranItemList(params);
}
