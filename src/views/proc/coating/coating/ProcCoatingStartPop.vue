...
<Card style="width: 34rem; height: 21rem;">
    <template #content>
        <div class="grid mb-2">
            ...
            <div class="col-12">
                <FloatLabel variant="on">
                    <Select v-model="form.managerName" :options="workerList"
                    optionLabel="codeNm"
                    optionValue="code"
                    class="w-full"
                    />
                    <label for="on_label">담당자</label>
                </FloatLabel>
            </div>
        </div>
...
const filteredStorages = computed(() => {
    ...
    return allStorages.value.filter(s => s.areaCd === form.areaCd && s.procCd === 'PRC003');
});
    ...
    confirm.require({
        ...
        accept: async () => {
            try {
                ...
                const res = await ApiProc.startProcCoating(param)
                vSuccess('코팅작업이 가능합니다.')
                closeDialog()
            } catch (err) {
                handleApiError(err)
            }
...
onMounted( async () =>{
    ...
    workerList.value = res.data.map(item => ({
        codeNm: item.workerName,
        code: item.workerName
    }));
