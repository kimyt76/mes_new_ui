...
const moveReq = () =>{
    if( selectedItem.value.length <=0 ) return vWarning('이동요청할 자재를 선택해주세요.')
...
const downloadExcel = () =>{
  ...
  exportToExcel(chargeList.value, "충전 리스트", cols);
}
