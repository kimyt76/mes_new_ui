...
<div class="w-full flex gap-2 justify-end mt-3">
    <Button
        label="저장"
        severity="secondary"
        @click="saveInfo"
    ...
    <Dialog
        v-model:visible="purchaseOrderListDialog"
        header="발주서 조회"
        modal
        :draggable="false"
        :resizable="false"
        ...
        <PurchaseOrderListPop
            @@@selected = "handleSelected()"
            @close="purchaseOrderListDialog=false"
            />
    </Dialog>
...
const purchaseOrderListDialog = ref(false)
...
const itemPop = (type) =>{
    if ( type === 'O' ){
        purchaseOrderListDialog.value = true
    }else{
